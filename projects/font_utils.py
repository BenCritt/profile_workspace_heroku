...
import io, tinycss2, requests, cssutils, time, sys, re, pathlib, os, logging, json, csv
...
def report_to_csv(rows: list[dict]) -> io.BytesIO:
    ...
    return out
# ───────────────────────────────────────────────────────────
# Font Inspector: async queue + real progress + disk files
# ───────────────────────────────────────────────────────────
import os, uuid, threading, queue, time, tempfile, shutil
...
# Where to write CSVs on disk
_FILES_DIR = os.path.join(tempfile.gettempdir(), "font_inspector_reports")
os.makedirs(_FILES_DIR, exist_ok=True)

_CSV_TTL = timedelta(minutes=30)  # delete files older than 30 minutes
...
_QUEUE: "queue.Queue[Task]" = queue.Queue(maxsize=20)
...
def _queue_pos(task_id: str) -> Optional[int]:
    ...
    if t.status != "queued":
        return 0
    # how many queued before me?
    return sum(1 for x in _TASKS.values()
               if x.status == "queued" and x.created_at < t.created_at)

def _cleanup_stale_files() -> None:
    """Delete CSV files older than TTL and prune tasks pointing to them."""
    cutoff = datetime.now(timezone.utc) - _CSV_TTL
    try:
    ...
    # prune task records with missing/expired files
    dead_ids: List[str] = []
...
def _unique_csv_path(task_id: str) -> str:
    # Example: font_report_2025-09-01_123456Z_abc123.csv
    ts = datetime.utcnow().strftime("%Y-%m-%d_%H%M%SZ")
    filename = f"font_report_{ts}_{task_id[:8]}.csv"
    return os.path.join(_FILES_DIR, filename)

def _worker():
    while True:
        task: Task = _QUEUE.get()
        try:
            # Phase 1: validate
            task.status, task.progress, task.message = "running", 5, "Validating URL…"

            # Phase 2: fetch page & analyze
            task.progress, task.message = 25, "Fetching page…"
            rows = make_report(task.url)  # <- your existing function
            task.progress, task.message = 80, "Analyzing fonts & licenses…"


            # Phase 2.5: Save rows on the task so the table can be rendered in the template.
            task.rows = rows or []

            # Phase 3: write CSV to disk
            task.message = "Preparing CSV…"
            csv_io = report_to_csv(rows or [])  # returns BytesIO (existing helper)
            path = _unique_csv_path(task.id)
            with open(path, "wb") as f:
                f.write(csv_io.getvalue())

            task.file_path = path
            task.progress, task.status, task.message = 100, "done", "Ready"
...
def _ensure_worker():
    ...
    with _LOCK:
        if not _WORKER_STARTED:
            th = threading.Thread(target=_worker, name="font-inspector-worker", daemon=True)
            th.start()
            _WORKER_STARTED = True

@require_POST
def start_font_scan(request: HttpRequest):
    """POST: validate, enqueue, return task id + initial status."""
    ...
    t = Task(id=str(uuid.uuid4()), url=url)
    _TASKS[t.id] = t
    _QUEUE.put(t)
...
def get_font_task_status(request: HttpRequest, task_id: str):
    ...
    if t.status == "error":
        ...
        # Always include a preview of rows so the client can render the table.
        payload["rows"] = (t.rows or [])[:500]
        # Only expose a download URL if the file still exists.
...
def download_font_task_file(request: HttpRequest, task_id: str):
    """GET: stream CSV and delete it (now or soon). Also hide the Download button on client."""
    _cleanup_stale_files()
    ...
    # Mark as downloaded so the client can hide the button on its next poll (or immediately after click)
    t.downloaded = True

    # Stream file and schedule deletion shortly after serving
    path = t.file_path
    filename = os.path.basename(path)

    def _delete_later(p: str):
        # give the webserver a little time to finish sending the response
        def _kill():
            time.sleep(10)
            try:
                os.remove(p)
