...
#
# Changes (2026-07-19, latency fix — v4):
#   - Product bar: the original free-form worker delivers in ~31s; the
#     structured path must match or beat that, not hide behind timeouts.
#   - Root cause of structured-path latency (>120s vs ~31s free-form):
#     gemini-2.5-flash "thinks" by default on a dynamic budget, and
#     response_schema is a known trigger for very large hidden thinking
#     budgets — the model plans the entire constrained JSON before emitting
#     anything. Fix: thinking_budget=0 on the structured call (verified
#     supported on the pinned google-genai 1.67.0). The schema, not model
#     deliberation, is what guarantees the format, so consistency is
#     unaffected; if classification quality ever dips, raise the budget
#     modestly (e.g. 512) rather than reverting to dynamic.
#   - Watchdogs tightened to user-respecting values: structured 45s,
#     fallback 60s (fallback measured ~28–31s in production twice). SDK
#     HTTP timeout 55s. Typical run ~15–30s; worst case ~2 minutes ending
#     in a complete free-form report or a clean error.
#   - Every path now logs elapsed seconds; the structured-OK line also
#     logs thoughts_token_count / candidates_token_count so one production
#     run confirms or refutes the thinking diagnosis numerically.
#
...
def run_gemini_job(job_id: str, job_desc: str, gemini_key: str) -> None:
    ...
    SDK/config errors, network hangs (each call bounded by an
    SDK-independent watchdog — 45s structured, 60s fallback — on top of
    the SDK's own 55s HTTP timeout) —
    writes {"status": "error"} for the next poll to deliver. Typical run
    is ~15-30s (thinking disabled on the structured call); worst case is
    ~2 minutes (45s structured watchdog + 60s fallback watchdog), inside
    ...
    try:
        ...
        client = genai.Client(
            ...
            # seconds for httpx and sent as X-Server-Timeout). Sized to
            # cover the longest per-call watchdog below with margin.
        ...
        try:
            ...
            response = _generate_with_watchdog(
                ...
                config=types.GenerateContentConfig(
                    ...
                    # 2.5 Flash reasons ("thinks") by default on a dynamic
                    # budget; combined with response_schema that is a known
                    # latency trap — the model plans the whole constrained
                    # JSON in hidden thinking tokens before emitting
                    # (measured >120s here vs ~31s free-form). The schema
                    # guarantees the format, so thinking is disabled on
                    # this call.
                    thinking_config=types.ThinkingConfig(thinking_budget=1024),
                ),
                timeout_seconds=45,
            )
            structured_elapsed = time.monotonic() - t0
            ...
            print(
                f"JFA worker [{job_id}]: structured path OK "
                f"({structured_elapsed:.1f}s, "
                f"thoughts={getattr(_um, 'thoughts_token_count', None)}, "
                f"out={getattr(_um, 'candidates_token_count', None)})",
                flush=True,
            )
            ...
            print(
                f"JFA worker [{job_id}]: structured path failed after "
            ...
            print(
                f"JFA worker [{job_id}]: free-form fallback OK "
