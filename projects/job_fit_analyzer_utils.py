...
def run_gemini_job(job_id: str, job_desc: str, gemini_key: str) -> None:
    ...
    SDK/config errors, network hangs (bounded by a 120-second HTTP
    timeout) — writes {"status": "error"} for the next poll to deliver.
    ...
    try:
        # (2026-07-19 hardening) Everything below — imports included — runs
        # inside the try. Previously the markdown/textwrap/datetime imports
        # and the prompt construction ran before it, so any failure there
        # killed the thread without writing an error status. The cache
        # entry then sat at "pending" until TTL expiry, which the client
        # renders as an endless "Almost there…" spinner followed by a
        # job-expired message. Now every failure mode lands in the except
        # handler and becomes a visible error within one poll cycle.
        ...
        # Without this anchor, the model cannot resolve phrases like
        # "November 2023 to present" into a duration, and it will hedge on
        # minimum-experience requirements. Heroku dynos run UTC; day-level
        # precision is sufficient for tenure math, so no timezone
    ...
    **Development Portfolio (bencritt.net — 50+ interactive tools):**
    ...
    """).strip()
        ...
        client = genai.Client(
            api_key=gemini_key,
            # Hard cap on the HTTP call, in MILLISECONDS per the SDK's
            # HttpOptions contract (verified on the pinned google-genai
            # 1.67.0). Without it the SDK waits indefinitely on a stalled
            # connection — indistinguishable from silent thread death: the
            # job stays "pending" forever. With it, a hang becomes a caught
            # timeout exception and a visible error status after ~2 minutes.
            http_options=types.HttpOptions(timeout=120_000),
        )

        print(f"JFA worker [{job_id}]: calling Gemini", flush=True)
